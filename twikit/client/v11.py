...
class Endpoint:
    ...
    CONVERSATION_UPDATE_NAME = (
        f"https://{DOMAIN}/i/api/1.1/dm/conversation/{{}}/update_name.json"
    )
...
class V11Client:
    ...
    async def account_logout(self):
        return await self.base.post(
            Endpoint.ACCOUNT_LOGOUT, headers=self.base._base_headers
        )

    async def onboarding_task(
        self, guest_token, token, subtask_inputs, data=None, **kwargs
    ):
        if data is None:
            data = {}
    ...
    async def get_place(self, id):
        return await self.base.get(
            Endpoint.GET_PLACE.format(id), headers=self.base._base_headers
        )
    ...
    async def create_friendships(self, user_id):
        ...
        return await self.base.post(
            Endpoint.CREATE_FRIENDSHIPS, data=data, headers=headers
        )
    ...
    async def destroy_friendships(self, user_id):
        ...
        return await self.base.post(
            Endpoint.DESTROY_FRIENDSHIPS, data=data, headers=headers
        )
    ...
    async def guide(self, category, count, additional_request_params):
        ...
        return await self.base.get(
            Endpoint.GUIDE, params=params, headers=self.base._base_headers
        )

    async def available_trends(self):
        return await self.base.get(
            Endpoint.AVAILABLE_TRENDS, headers=self.base._base_headers
        )
    ...
    async def _friendships(self, user_id, screen_name, count, endpoint, cursor):
        ...
        return await self.base.get(
            endpoint, params=params, headers=self.base._base_headers
        )

    async def followers_list(self, user_id, screen_name, count, cursor):
        return await self._friendships(
            user_id, screen_name, count, Endpoint.FOLLOWERS_LIST, cursor
        )

    async def friends_list(self, user_id, screen_name, count, cursor):
        return await self._friendships(
            user_id, screen_name, count, Endpoint.FRIENDS_LIST, cursor
        )
    ...
    async def dm_inbox(self, max_id):
        ...
        return await self.base.get(
            Endpoint.DM_INBOX, params=params, headers=self.base._base_headers
        )
